<template>
  <v-container>
    <v-row justify="center">
      ...
      <v-col cols="10" sm="10">
        <v-form ref="secondFormNote" lazy-validation>
          <v-textarea
...
export default {
  ...
  created() {
    if (process.client) {
      const form = localStorage.getItem(constant.SECOND_FORM_NOTE)
  ...
  methods: {
    save() {
      localStorage.setItem(constant.SECOND_FORM_NOTE, JSON.stringify(this.form))
    },
    draft() {
      this.save()
    ...
    submit() {
      if (this.$refs.secondFormNote.validate()) {
        this.save()
        this.$router.push('/inspection')
      }
    }
